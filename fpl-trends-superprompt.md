...
### The export stops recomputing the archive, and refreshes every six hours — 2026-09-23

The graph pass found that `web.export all` spent 30 of its 32 seconds on `cmd_scorecard`, a walk-forward over the three archive seasons. That file cannot change unless the model, the backtest or the archive does, yet it was recomputed after every hourly collector run. Three changes:

- **One walk-forward, not two.** `run_component_decomposition` walked every season again only to recover event vectors the event model had already projected. `project_points` now takes an `on_projection` hook, and `analytics.evaluate.run_evaluation` returns the comparison results and the decomposition from a single pass. Verified identical to the old two-pass output (332,140 result rows, 83,035 decomposition rows, `assert_frame_equal` and exact list equality), in 15s instead of 25s. `run_comparison` and `run_component_decomposition` are gone; their callers (`web/export/scorecard.py`, `analytics/__main__.py`, `tests/test_model_comparison.py`) use `run_evaluation`. The test module now shares one fixture, so the suite went from about 49s to about 37s.
- **Only current-season work runs on the clock.** `scorecard` left `web.export all`. `web.yml` runs it on pushes, whose path filter already covers `analytics/**`, `backtest/**` and `data/historical/**`, and on manual runs, not on scheduled runs. The board, correlations and players exports still pool the archive, since their published accuracy depends on it, and together they take well under a second. The raw archive cache that `backtest.backfill` restores is now kept by `actions/cache`, keyed on `backfill.py`, instead of being downloaded every run.
- **Six-hourly, not after every collector run.** The operator reads the site a few times a day: *"Change the hourly for every 6 hours. I'm not checking it hourly."* `web.yml` runs on `cron: "20 */6 * * *"` instead of `workflow_run` of `collect`. The collector itself stays hourly, because a missed snapshot is lost for good (§0.1), and each refresh exports everything collected since the last one. The export now runs on its own clock, so the collector may push in between, and the commit step does `git pull --rebase` first. The collector never touches `data/web/v1`, so the rebase can't conflict.

A fresh scorecard built on Windows differs from the CI-built committed one in a single value, the 12th significant digit of one p-value. That's float noise between machines, and it was already there before this change.

### The frontend is redesigned as a modern FPL site, for desktop and phone — 2026-09-23

The operator asked for the frontend to *"look like a modern FPL site"*, with FPL Focal (fpl.page) as the reference, and to be usable on both desktop and phone. This supersedes §5.8.1's dark-only instrument look. That is an operator-directed change to the design system, not a deviation from it.

- **Tokens, not views.** Every view already styled itself through `src/design/tokens.css`, with no hard-coded colours anywhere, so most of the redesign is a new token set under the old names. Light is the default; dark follows the system setting or a header toggle that stamps `data-theme` on `<html>` (applied before first paint by `index.html`). The palette is Premier League purple (`--brand`) with the electric green (`--accent`), on lavender-grey pages with white cards. Barlow Condensed is used for headlines and big numbers, and Barlow for body text and data with tabular figures, replacing Archivo, Public Sans and IBM Plex Mono.
- **New semantic tokens.** `--good`/`--bad` (green up, red down, FPL's convention) are now separate from the teal/rose correlation poles, which say which way a relationship runs, not whether it is good. `--series-1..6` give player lines distinct colours. `--selected-bg`/`--selected-ink` handle selected toggles in both themes. `design/scale.ts:outcomeColor` is the good/bad counterpart of `divergingColor`, used for the fixture ticker (FDR green to red) and for Form Matrix metrics that have a direction. Points count as higher-is-better there, since the registry gives context columns no direction. Pass/fail, better/worse and transfers in/out moved from rho colours to good/bad.
- **Colour mixing moved from OKLCH to OKLab.** OKLCH interpolates hue, and mixing a pole into the purple-tinted card colour swung green through blue (hue 237). OKLab mixes straight across. `scale.test.ts` now asserts a perceptual space rather than OKLCH specifically.
- **Navigation.** The eleven surfaces are grouped into five sections (`surfaces.ts:SECTIONS`: Model, Players, Fixtures, My Team, Lab). The purple header carries the sections and a pill strip carries the current section's pages. On phones (≤760px, `design/useNarrow.ts`) the sections become a bottom tab bar and the strip scrolls the active pill into view. The default view is now the Model Board (`url.ts:DEFAULT_VIEW`).
- **Phone fixes found by screenshotting every surface at 390px.** Model Board accuracy rows stack. Explorer pins the player column and lets the page scroll. The shared `FilterBar` collapses behind a "Filters" button showing how many filters are on. The Correlation Lab's row labels truncate and stay pinned, and a season picker that did not wrap was forcing the whole page 33px wider than the screen. Trend Explorer draws at 380 units on phones instead of 900, and the shared chart's labels scale up. Graph Builder's column list is capped.
- **§5.8.6's model wall is kept, structurally.** Model-authored content sits in raised cards with the `model_git_sha` attribution. The full-page panel with a left rule would have been a stray stripe in a card layout.

All 177 frontend tests pass and the typecheck is clean. The initial chunk is 94.9 KB gzipped, against 93.6 KB before and a 250 KB budget. Vite's raw-size warning on that chunk was already there.
